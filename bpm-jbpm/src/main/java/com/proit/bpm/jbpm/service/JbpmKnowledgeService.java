...
public class JbpmKnowledgeService implements ApplicationListener<WorkflowsUpdatedEvent>
{
	...
	private synchronized void initialize()
	{
		...
		catch (Exception e)
		{
			...
			throw new AppException(ApiStatusCodeImpl.ILLEGAL_INPUT, "failed to load workflow definitions", e);
		}
	}
	...
	private KieBase readKnowledgeBase()
	{
		...
		workflowService.getWorkflows().stream().filter(it -> it.getEngineType() == EngineType.JBPM).forEach(workflow -> {
