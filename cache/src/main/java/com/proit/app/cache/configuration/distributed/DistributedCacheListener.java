	...
	public void onItemReceived(DistributedCacheItem item)
	{
		try
		{
			...
			if (!StringUtils.equals(item.getSourceNode(), nodeId))
			{
				...
				var cacheOperations = cacheProperties.getDistributedOperations().get(item.getCacheName());
				...
				if (cacheOperations != null)
				{
					...
					if (targetCache != null)
					{
						...
						{
							if (item.getValue() != null && DistributedOperationsHelper.isPropagatingPut(cacheOperations))
							{
