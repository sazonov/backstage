...
@Tag(name = "remote-dict-data-service-v2", description = "Методы для работы с данными справочников")
@FeignClient(value = RemoteServices.DICTS, path = "/api/remote/dicts/v2", contextId = "remoteDictDataServiceV2")
public interface RemoteDictDataServiceV2
{
	...
	ApiResponse<List<DictItemDto>> getByIds(@PathVariable String dictId, @RequestBody List<String> ids);

	@Schema(description = "Добавление записи в справочник.")
	@PostMapping("/{dictId}/create")
	ApiResponse<DictItemDto> create(@PathVariable String dictId, @RequestBody CreateDictItemRequest request);

	@Schema(description = "Обновление записи в справочнике.")
	@PostMapping("/{dictId}/update")
	ApiResponse<DictItemDto> update(@PathVariable String dictId, @RequestBody UpdateDictItemRequest request);
	...
	ApiResponse<Long> countByFilter(@PathVariable String dictId, @RequestParam String filter);
