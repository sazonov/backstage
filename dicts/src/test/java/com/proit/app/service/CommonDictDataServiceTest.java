...
public class CommonDictDataServiceTest extends CommonTest
{
	...
	protected void getByFilter()
	{
		var result = dictDataService.getByFilter(TESTABLE_DICT_ID, List.of("*"),
				"stringField like 'str' and (integerField = 1 or integerField in (2, 5, 8) and integerField != 10 or integerField <= 2 and doubleField > 1.9 and doubleField < 2.1)",
				PageRequest.of(0, 10));
	...
	protected void getByFilterInnerDictSort()
	{
		...
		var result = dictDataService.getByFilter(TESTABLE_REF_DICT_ID, List.of("*", TESTABLE_DICT_ID + ".*"), null,
	...
	protected void attachmentBindingWithCreateDictItem()
	{
		...
		var attachments = attachmentService.getAttachments(
				DICTS_ATTACHMENT_TYPE.formatted(TESTABLE_ATTACH_DICT_ID, dictItem.getId(), "attachmentField"), dictItem.getId());
	...
	protected void checkAttachmentBindingWithUpdateDictItem()
	{
		...
		var attachments = attachmentService.getAttachments(
				DICTS_ATTACHMENT_TYPE.formatted(TESTABLE_ATTACH_DICT_ID, updatedDictItem.getId(), "attachmentField"), updatedDictItem.getId());
	...
	protected void checkAttachmentReleaseWithDeleteDictItem()
	{
		...
		dictDataService.delete(TESTABLE_ATTACH_DICT_ID, dictItem.getId(), true);
	...
	protected void checkAttachmentBindingWithDeleteDictItem()
	{
		...
		dictDataService.delete(TESTABLE_ATTACH_DICT_ID, dictItem.getId(), false);
	...
	protected void deleteDictItem()
	{
		...
		dictDataService.delete(TESTABLE_DICT_ID, dictItem.getId(), true);
		assertNotNull(dictDataService.getById(TESTABLE_DICT_ID, dictItem.getId()).getDeleted());

		dictDataService.delete(TESTABLE_DICT_ID, dictItem.getId(), false);
	...
	protected void deleteWithReason(String reason)
	{
		...
		dictDataService.delete(TESTABLE_DICT_ID, dictItem.getId(), true, reason);
