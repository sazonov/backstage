...
public class CommonDictServiceTest extends CommonTest
{
	...
	{
		updateWithoutRenameDict(dictId);
		updateWithRenameDict(dictId);
	}

	protected void updateWithoutRenameDict(String dictId)
	{
	...
	protected void updateWithRenameDict(String dictId)
	{
		var d = createNewDict(dictId);

		var updateDict = dictService.getById(d.getId());

		updateDict.setId("updated_" + d.getId());
		updateDict.setName("updated_" + d.getId());

		updateDict.setFields(new ArrayList<>(withoutServiceFields(d.getFields())));

		var expected = dictService.update(d.getId(), updateDict);
	...
	protected void createWithUUIDDictFieldIds()
	{
		...
		dict.setFields(dict.getFields()
				.stream()
				.peek(it -> it.setId(RandomStringUtils.randomNumeric(1) + StringUtils.substring(UUID.randomUUID().toString(), 1)))
	...
	private List<DictField> withoutServiceFields(List<DictField> source)
	{
