...
@Service
public class DictDataValidationServiceImpl implements DictDataValidationService
{
	...
	public DictDataValidationServiceImpl(DictService dictService, @Lazy DictDataService dictDataService, ObjectMapper objectMapper)
	{
	...
	{
		var scheme = dictService.getById(dictId);
	...
	@Override
	public void validateDictDataItem(DictDataItem dataItem)
	{
		...
		validateRequiredFields(dataItem.getDictId(), dataItemMap, availableFields);
	}

	private void validateRequiredFields(String dictId, Map<String, Object> dictData, List<DictField> availableFields)
	{
		availableFields.stream()
				.filter(field -> !ServiceFieldConstants.getServiceInsertableFields().contains(field.getId()))
				.peek(dictField -> checkCast(dictId, dictField, dictData.get(dictField.getId())))
	...
	private void checkSingleElementCast(String dictId, DictField dictField, Object value)
	{
		try
		{
			switch (dictField.getType())
			{
				...
				case DICT -> {
					var r = dictDataService.getByFilter(
							dictField.getDictRef().getDictId(),
							List.of("id"),
