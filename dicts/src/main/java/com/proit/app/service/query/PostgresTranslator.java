...
import com.proit.app.model.postgres.query.PostgresQueryField;
...
public class PostgresTranslator implements Translator<PostgresQueryContext>, Visitor<TranslationResult<?>>
{
	...
	public TranslationResult<PostgresQueryContext> visit(LogicQueryExpression expression, TranslationContext context)
	{
		...
		{
			case AND -> query = new PostgresQueryContext(String.join(" and ", leftExpression, rightExpression), participantDictIds);
			case OR -> query = new PostgresQueryContext(String.join(" or ", leftExpression, rightExpression), participantDictIds);
