...
package com.proit.app.service.migration;

import com.proit.app.configuration.backend.provider.DictSchemeBackendProvider;
import com.proit.app.configuration.backend.provider.DictTransactionBackendProvider;
...
public class DictTransactionProvider
{
	...
	public void addDictTransactionItem(DictTransactionItem item)
	{
		if (isActiveTransaction())
		{
			if (item.isSchemeUsed() && notContainsOriginal(DICT_SCHEME_ID))
			{
				...
				addAffectedDict(DictTransactionItem.of(DICT_SCHEME_ID, dictCopyId,
						dictEngine, item.isSchemeUsed()));
			}

			if (item.getOriginalDictId() != null && notContainsOriginal(item.getOriginalDictId()))
			{
				var dictCopyId = dictTransactionBackendProvider.getBackendByEngineName(item.getDictEngine().getName())
						.copyDict(item.getOriginalDictId());

				addAffectedDict(DictTransactionItem.of(item.getOriginalDictId(), dictCopyId,
						item.getDictEngine(), item.isSchemeUsed()));
			}
		}
	}
