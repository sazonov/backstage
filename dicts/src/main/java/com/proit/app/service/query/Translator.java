...
public class Translator implements Visitor<TranslationResult<?>>
{
	public Criteria process(Dict dict, Expression expression)
	{
		return expression.process(this, new TranslationContext(dict)).value(Criteria.class);
	}
	...
	public TranslationResult<String> visit(Field field, TranslationContext context)
	{
		context.dict()
				.getFields()
				.stream()
				.filter(it -> it.getId().equalsIgnoreCase(field.name) || field.name.equals(ID))
				.findFirst()
				.orElseThrow(() -> new FieldNotFoundException(context.dict().getId(), field.name));

		return new TranslationResult<>(field.name);
	}

	public TranslationResult<Criteria> visit(LikeExpression expression, TranslationContext context)
	{
	...
	public TranslationResult<Criteria> visit(IlikeExpression expression, TranslationContext context)
	{
		var field = expression.field.process(this, context);
		var template = expression.template.process(this, context);
	...
	public TranslationResult<Criteria> visit(LogicExpression expression, TranslationContext context)
	{
	...
	public TranslationResult<Criteria> visit(InExpression expression, TranslationContext context)
	{
