...
import static com.proit.app.model.dictitem.DictItemColumnName.SERVICE_COLUMNS;
...
public class PostgresDictDataBackendMapper implements DictDataBackendMapper<PostgresDictItem>
{
	...
	public DictItem mapFrom(String dictId, PostgresDictItem source)
	{
		try
		{
			...
			var dataItemMap = source.getDictData()
					.entrySet()
					.stream()
					.collect(Collectors.groupingBy(it -> dictId(it.getKey()),
							StreamCollectors.toLinkedHashMap(it -> fieldId(it.getKey()), Map.Entry::getValue)));

			var mappedDictData = mappedDataItems(dataItemMap, dataFieldMap, refFieldMap);
			...
			return DictItem.builder()
					...
					.created(localDateTime(source.getCreated()))
					.updated(localDateTime(source.getUpdated()))
					.deleted(localDateTime(source.getDeleted()))
	...
	private String dictId(String placeholder)
	{
		return StringUtils.substringBefore(placeholder, "__");
	}

	private String fieldId(String placeholder)
	{
		return StringUtils.substringAfter(placeholder, "__");
	}

	private Map<String, Object> mappedDataItems(Map<String, Map<String, Object>> dataItemMap,
	                                            Map<String, DictField> dataFieldMap, Map<String, DictField> refFieldMap)
	{
		...
		dataItemMap.forEach((id, dictData) -> {
			if (refFieldMap.containsKey(id))
			{
				var refDataMap = dictData.entrySet()
						.stream()
						.collect(StreamCollectors.toLinkedHashMap(it -> "%s__%s".formatted(id, it.getKey()), Map.Entry::getValue));

				var refDictId = refFieldMap.get(id).getDictRef().getDictId();

				var refDictItem = mapFrom(refDictId, new PostgresDictItem(refDictId, refDataMap));
			...
			var mappedData = dictData.entrySet()
					.stream()
					.filter(it -> !refFieldMap.containsKey(it.getKey()))
					.filter(it -> !SERVICE_COLUMNS.contains(it.getKey()))
	...
	private Object mapMultipleValue(DictField field, Object value)
	{
		try
		{
			if (field.getType() == DictFieldType.JSON)
			{
				...
				return JsonUtils.attributeValueMaps(jsonb.getValue())
						.stream()
	...
	private Object mapSingleValue(Object value, DictField field)
	{
		...
		{
			return value == null ? Collections.emptyMap() : JsonUtils.attributeValueMap(value);
		}
			...
			return switch (field.getType())
					{
						...
						case TIMESTAMP -> localDateTime(date);
	...
	{
		return value == null ? null : Instant.ofEpochMilli(value.getTime()).atZone(ZoneId.systemDefault()).toLocalDateTime();
	}
